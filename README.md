# 🚀 Raspberry Pi First-Boot Setup Scripts
...
#### Bash (`setup.sh`)
```sh
bash <(curl -fsSL https://raw.githubusercontent.com/corechunk/rpi_server/main/setup.sh)
```

#### POSIX (`POSIX.sh`)
```sh
sh -c "$(curl -fsSL https://raw.githubusercontent.com/corechunk/rpi_server/main/POSIX.sh)"
```
